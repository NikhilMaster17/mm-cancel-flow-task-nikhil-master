...
export default function ProfilePage() {
  ...
  // Add state for cancellation flow
  const [showCancelFlow, setShowCancelFlow] = useState(false);
  const [cancelStep, setCancelStep] = useState(0);

  // Add state for A/B variant
  const [downsellVariant, setDownsellVariant] = useState<'A' | 'B' | null>(null);
  const userId = mockUser.id;

  // Secure RNG for A/B assignment
  function assignVariant() {
    // Use crypto for secure RNG
    const array = new Uint32Array(1);
    window.crypto.getRandomValues(array);
    return array[0] % 2 === 0 ? 'A' : 'B';
  }

  // Check or assign downsell variant on flow start
  const handleStartCancelFlow = async () => {
    // Query for existing cancellation for user
    ...
    if (data && data.length > 0) {
      variant = data[0].downsell_variant;
    } else {
      variant = assignVariant();
      // Persist variant with stub cancellation record
  ...
  // Handler for closing modal/flow
              ...
              <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 relative">
                ...
                {cancelStep === 0 && (
                  <div>
                    ...
                    <p className="mb-6">Are you sure you want to cancel your Migrate Mate subscription?</p>
                    <button
                ...
                {cancelStep === 1 && (
                  <div>
                    <h2 className="text-lg font-bold mb-4">Tell us why</h2>
                    <p className="mb-6">Please let us know your reason for cancelling (step placeholder).</p>
                    <button
                      className="w-full py-2 px-4 bg-gray-200 text-gray-700 rounded-lg mt-2"
                      onClick={handleCloseCancelFlow}
                    >
                      Back
                    </button>
                    <button
                      className="w-full py-2 px-4 bg-[#8952fc] text-white rounded-lg mt-2"
                ...
                {cancelStep === 2 && (
                  <div>
                    <h2 className="text-lg font-bold mb-4">Final Step</h2>
                    <p className="mb-6">Downsell/confirmation step placeholder.</p>
                    <button
                      className="w-full py-2 px-4 bg-gray-200 text-gray-700 rounded-lg mt-2"
                      onClick={() => setCancelStep(1)}
                    >
